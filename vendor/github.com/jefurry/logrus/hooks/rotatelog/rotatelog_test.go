...
func TestSatisfiesIOWriter(t *testing.T) {
	var w io.Writer
	w, _ = rotatelog.New("/foo/bar")
	_ = w
}

func TestSatisfiesIOCloser(t *testing.T) {
	var c io.Closer
	c, _ = rotatelog.New("/foo/bar")
	_ = c
}
...
func TestLogRotate(t *testing.T) {
	...
	rl, err := rotatelog.New(
		filepath.Join(dir, "log%Y%m%d%H%M%S"),
		rotatelog.WithClock(clock),
		rotatelog.WithMaxAge(24*time.Hour),
		rotatelog.WithLinkName(linkName),
	)
	if !assert.NoError(t, err, `rotatelog.New should succeed`) {
		return
	}
	defer rl.Close()
...
func TestLogRotationCount(t *testing.T) {
	dir, err := ioutil.TempDir("", "file-rotatelog-rotationcount-test")
	if !assert.NoError(t, err, "creating temporary directory should succeed") {
		return
	}
	defer os.RemoveAll(dir)
	...
	t.Run("Either maxAge or rotationCount should be set", func(t *testing.T) {
		rl, err := rotatelog.New(
			filepath.Join(dir, "log%Y%m%d%H%M%S"),
			rotatelog.WithClock(clock),
			rotatelog.WithMaxAge(time.Duration(0)),
			rotatelog.WithRotationCount(0),
		)
...
func TestLogSetOutput(t *testing.T) {
	...
	rl, err := rotatelog.New(filepath.Join(dir, "log%Y%m%d%H%M%S"))
	if !assert.NoError(t, err, `rotatelog.New should succeed`) {
		return
	}
	defer rl.Close()
...
func TestGHIssue16(t *testing.T) {
	...
	dir, err := ioutil.TempDir("", "file-rotatelog-gh16")
	if !assert.NoError(t, err, `creating temporary directory should succeed`) {
		return
	}
	defer os.RemoveAll(dir)
	...
	rl, err := rotatelog.New(
		...
		rotatelog.WithRotationTime(10*time.Second),
		rotatelog.WithRotationCount(3),
		rotatelog.WithMaxAge(-1),
	)
	if !assert.NoError(t, err, `rotatelog.New should succeed`) {
		return
...
func TestRotationGenerationalNames(t *testing.T) {
	dir, err := ioutil.TempDir("", "file-rotatelog-generational")
	if !assert.NoError(t, err, `creating temporary directory should succeed`) {
		return
	}
	defer os.RemoveAll(dir)
	...
	t.Run("Rotate over pattern change over every second", func(t *testing.T) {
		rl, err := rotatelog.New(
			...
			rotatelog.WithRotationTime(time.Nanosecond),
		)
		if !assert.NoError(t, err, `rotatelog.New should succeed`) {
			return
		}
		...
		for _, test := range tests {
			t.Run(fmt.Sprintf("location = %s, time = %s", locName, test.Clock.Now().Format(time.RFC3339)), func(t *testing.T) {
				...
				rl, err := rotatelog.New(
					...
					rotatelog.WithClock(test.Clock), // we're not using WithLocation, but it's the same thing
				)
				if !assert.NoError(t, err, "rotatelog.New should succeed") {
					return
				}
