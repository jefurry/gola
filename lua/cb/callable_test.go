...
func TestCallable_1(t *testing.T) {
	...
	for _, v := range []struct {
		...
		if !assert.Equal(t, v.ref, callable.Ref(), "v.ref must be equals to Ref") {
			return
		}

		objFn, err := callable.ObjFn(L)
		if !assert.NoError(t, err, "ObjFn should succeed") {
			return
		}

		if !assert.NotEqual(t, objFn, nil, "ObjFn should succeed") {
			return
		}
