...
// Package filepath implements path/filepath for Lua.
package filepath
...
var filepathFields = map[string]lua.LValue{
	"Separator":     lua.LString(ffilepath.Separator),
	"ListSeparator": lua.LString(ffilepath.ListSeparator),
}

func filepathAbs(L *lua.LState) int {
	p := L.CheckString(1)

	s, err := ffilepath.Abs(p)
	if err != nil {
		L.Push(lua.LNil)
...
func filepathBase(L *lua.LState) int {
	...
	L.Push(lua.LString(ffilepath.Base(p)))
...
func filepathClean(L *lua.LState) int {
	...
	L.Push(lua.LString(ffilepath.Clean(p)))
...
func filepathDir(L *lua.LState) int {
	...
	L.Push(lua.LString(ffilepath.Dir(p)))
...
func filepathEvalSymlinks(L *lua.LState) int {
	...
	s, err := ffilepath.EvalSymlinks(p)
	if err != nil {
		L.Push(lua.LNil)
		L.Push(lua.LString(err.Error()))
...
func filepathExt(L *lua.LState) int {
	...
	L.Push(lua.LString(ffilepath.Ext(p)))
...
func filepathFromSlash(L *lua.LState) int {
	...
	L.Push(lua.LString(ffilepath.FromSlash(p)))
...
func filepathGlob(L *lua.LState) int {
	...
	matches, err := ffilepath.Glob(pattern)
	if err != nil {
		L.Push(lua.LNil)
		L.Push(lua.LString(err.Error()))
...
func filepathHasPrefix(L *lua.LState) int {
	...
	L.Push(lua.LBool(ffilepath.HasPrefix(p, prefix)))
...
func filepathIsAbs(L *lua.LState) int {
	...
	L.Push(lua.LBool(ffilepath.IsAbs(p)))
...
func filepathJoin(L *lua.LState) int {
	...
	L.Push(lua.LString(ffilepath.Join(ps...)))
...
func filepathMatch(L *lua.LState) int {
	...
	matched, err := ffilepath.Match(pattern, name)
	if err != nil {
		L.Push(lua.LFalse)
		L.Push(lua.LString(err.Error()))
...
func filepathRel(L *lua.LState) int {
	...
	s, err := ffilepath.Rel(basepath, targpath)
	if err != nil {
		L.Push(lua.LNil)
		L.Push(lua.LString(err.Error()))
...
func filepathToSlash(L *lua.LState) int {
	...
	L.Push(lua.LString(ffilepath.ToSlash(p)))
...
func filepathVolumeName(L *lua.LState) int {
	...
	L.Push(lua.LString(ffilepath.VolumeName(p)))
...
func filepathWalk(L *lua.LState) int {
	...
	err := ffilepath.Walk(root, func(p string, info os.FileInfo, err error) error {
		if err != nil {
			return err
		}
